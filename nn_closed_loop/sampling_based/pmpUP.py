...
class pmpUP():
    ...
    def get_error(self, input_constraint, reachable_sets, t_max=5):
        ...
        # ground truth
        true_hulls_monte_carlo = self.dynamics.get_sampled_convex_hull(
            input_constraint,
            t_max * self.dynamics.dt,
            controller=self.controller,
            method='monte_carlo')
        ...
        for t in range(t_max):
            true_hull_mc = true_hulls_monte_carlo[t+1]
            true_hull = true_hulls[t+1]
            estimated_hull = reachable_sets[t]
            estimated_hull = ConvexHull(
                estimated_hull.points[estimated_hull.vertices,:2])
            ...
            # we use the extremal method since
            # it is significantly more accurate
            true_hull = ConvexHull(true_hull.points[true_hull.vertices,:2])
            haus_dist = Hausdorff_dist_two_convex_hulls(true_hull, estimated_hull)
