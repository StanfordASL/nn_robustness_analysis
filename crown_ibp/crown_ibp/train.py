    ...
    for i, (data, labels) in enumerate(loader): 
        ...
        if verbose or method != "natural":
            if kwargs["bound_type"] == "convex-adv":
                ...
                _, lb, _, _ = model(norm=norm, x_U=data_ub, x_L=data_lb, eps=eps, C=c, upper=False, lower=True, method_opt="full_backward_range")
                ...
                if beta < 1e-5:
                    ...
                    if kwargs["runnerup_only"]:
                        ...
                        _, clb, _, bias = model(norm=norm, x_U=data_ub, x_L=data_lb, eps=eps, C=c, method_opt="backward_range")
            ...
            elif kwargs["bound_type"] == "interval-mfe":
                ub, lb = model(norm=norm, x_U=data_ub, x_L=data_lb, eps=eps, C=c, method_opt="interval_range-mfe")
                unstable = dead = alive = relu_activity = torch.tensor([0])
            elif kwargs["bound_type"] == "crown-mfe":
                ub, lb = model(norm=norm, x_U=data_ub, x_L=data_lb, eps=eps, C=c, upper=False, lower=True, method_opt="full_backward_range-mfe")
                unstable = dead = alive = relu_activity = torch.tensor([0])
            else:
